...
**Quoi** — `cloudflare-worker/src/index.js`, deux tables seulement : `LEGACY_REDIRECTS` (19 clés ajoutées, 12 cibles corrigées) et `GONE_PATHS` (4 clés retirées). `next.config.ts` : `/de-ch/industrie/<slug>` → `/de-ch/branchen/<slug allemand>` en 301, repli sur `/de-ch/branchen`. Deux fichiers de test.
...
**Effet attendu** — Après fusion (Vercel, ~3 min) : les 16 URL `/de-ch/industrie/<slug FR ou allemand>` des 8 secteurs suisses mènent à leur page `branchen` en un saut 301 ; les autres à `/de-ch/branchen`. Après déploiement du Worker (GO séparé) : 14 chemins de l'annexe K qui finissaient en 404 et 4 chemins en 410 redirigent en 301 vers une page 200 ; 2 bascules `/en` → `/fr` sur l'annexe K et 2 sur le lot C ; 7 redirections quittent la fiche `delisted`. Lisible dans la couverture GSC à J+14.
...
- *Simulation du Worker* (import du module, origine interceptée) sur 38 chemins, avant et après : 0 écart à l'attendu ; `/`, `/de/studio-photo/alphashot-xl`, `/industrie-defense` inchangés. Comparaison des tables avant/après : seules `LEGACY_REDIRECTS` et `GONE_PATHS` bougent.
...
- `node --check` vert. `npx tsc --noEmit` vert. `npm run test:unit` : 10 fichiers, **183 tests** verts (122 avant). `npx next build` vert (R1).

**Écarts entre la consigne et le dépôt (R7)** —
1. « Les 10 entrées dont la cible est `alphashot-xl-v2` » : 7 sont dans `LEGACY_REDIRECTS` et ont été corrigées. Les 3 autres sont dans des tables que la consigne interdit de toucher : `PRODUCT_REDIRECTS` (`/product/photo-studio-r3`) et `LANG_SPECIFIC_REDIRECTS` (`/es/studio-photo/alphashot-xl`, `/nl/studio-photo/alphashot-xl`). **Non modifiées**, signalées dans la PR avec le correctif proposé.
...
**Suite** — GO de Laurent pour le déploiement du Worker, puis `curl.exe` sur les témoins listés dans la PR. Décision sur les 3 entrées `alphashot-xl-v2` hors `LEGACY_REDIRECTS`. Pilote C6 au cycle suivant.
