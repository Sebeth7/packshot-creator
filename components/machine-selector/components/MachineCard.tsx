...
const AUTOMATION_LABELS: Record<string, { fr: string; en: string; color: string }> = {
  'manual': { fr: 'Manuel', en: 'Manual', color: 'bg-gray-100 text-gray-700' },
  ...
  return (
    <div
      className={`
        ...
        ${isSelected
          ? 'border-brand-red shadow-lg ring-2 ring-brand-red/20'
    ...
    >
      {/* Badge sélectionné */}
      {isSelected && (
        <div className="absolute top-3 right-3 z-10 bg-brand-red text-white text-xs font-semibold px-2 py-1 rounded-full">
      ...
      <div className="p-4">
        ...
        {onViewDetails && (
          <button
            ...
            className="w-full py-2 px-4 text-sm font-medium text-brand-red border border-brand-red rounded-lg hover:bg-brand-red hover:text-white transition-colors"
          >
