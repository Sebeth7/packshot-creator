...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: lang, namespace: 'packshotIndustriel' });

  return {
    title: t('meta.title'),
    description: t('meta.description'),
    alternates: {
      canonical: `https://www.packshot-creator.com/${lang}/packshot-industriel`,
      languages: {
        fr: '/fr/packshot-industriel',
        en: '/en/packshot-industriel',
      },
    },
    openGraph: {
      title: t('meta.title'),
...
const BENEFIT_ICONS = [Wrench, RotateCw, Lightbulb, Repeat, Plug];

const MACHINE_IDS = ['alphashot-xl-v2', 'alphashot-pro-g2'];
...
export default async function PackshotIndustrielPage({ params }: PageProps) {
  ...
  const breadcrumbs = [
    ...
    { name: t('hero.title').split(':')[0].trim(), url: `https://www.packshot-creator.com/${lang}/packshot-industriel` },
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-20 lg:py-28">
          <FadeInView className="max-w-3xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 bg-white/10 rounded-full px-4 py-1.5 text-sm text-very-peri-200 mb-6">
              ...
              {isFr ? 'Industrie & Technique' : 'Industry & Technical'}
            </div>
